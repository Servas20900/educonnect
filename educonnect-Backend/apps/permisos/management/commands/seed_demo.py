...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        # Ensure core system objects (roles, permisos, admin user) exist first
        self._seed_system(options)

        roles = self._roles_map()

        # Robustly obtain the admin user created by seed_system. Use environment
        # variables if provided, fallback to the default 'admin' username and
        # finally to any existing superuser.
        admin_username = os.getenv("SEED_ADMIN_USER", "admin")
        admin_email = os.getenv("SEED_ADMIN_EMAIL", "admin@mep.go.cr")

        admin, created = AuthUsuario.objects.get_or_create(
        ...
        admin.is_staff = True
        admin.save(update_fields=["email", "password", "is_superuser", "is_staff"])
    ...
    # --- Inlined seed_system functionality ---
